...
from utils import ConsecutiveMobiusKernel, CompleteMobiusKernel, MyCompleteMobiusKernel
...
pruning_idx = pos_x_data.shape[1] + -8


nsamples = 2**(pos_x_data.shape[1]-pruning_idx+1)

explainer = TimeShapKernel(f_hs, average_event, 42, "event")
true = explainer.shap_values(pos_x_data, nsamples=2**(pos_x_data.shape[1]-pruning_idx+1), pruning_idx=pruning_idx)

explainer = ConsecutiveMobiusKernel(f_hs, average_event, 42, "event")
consecutive = explainer.shap_values(pos_x_data, nsamples=nsamples, pruning_idx=pruning_idx)

print(f'mae consecutive: {mean_absolute_error(true, consecutive)}')

explainer = CompleteMobiusKernel(f_hs, average_event, 42, "event")
complete = explainer.shap_values(pos_x_data, nsamples=nsamples, pruning_idx=pruning_idx)

print(f'mae complete: {mean_absolute_error(true, complete)}')

explainer = MyCompleteMobiusKernel(f_hs, average_event, 42, "event")
my_complete = explainer.shap_values(pos_x_data, nsamples=nsamples, pruning_idx=pruning_idx)

print(f'mae my_complete: {mean_absolute_error(true, my_complete)}')
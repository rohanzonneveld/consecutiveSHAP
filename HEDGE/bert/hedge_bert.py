...
class HEDGE:
    ...
    def shapley_topdown_tree(self, model, inputs, win_size):
        ...
        for level in range(1, self.fea_num):
            ...
            pos_opt = 0
            inter_idx_opt = 0
            while pos < len(fea_set): # go over all text spans
                subset = fea_set[pos]
                sen_len = len(subset)
                if sen_len == 1:
                    pos += 1
                    continue
                new_fea_set = [ele for x, ele in enumerate(fea_set) if x != pos] # delete text span at pos from fea_set
                score_buff = []
                for idx in range(1, sen_len): # go over all possible split points in text span and calculate SII

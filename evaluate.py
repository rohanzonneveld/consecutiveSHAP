...
def calc_metrics(method='MöbiusHEDGE', dataset='SST-2'):
    ...
    for ndir in range(nsamples):
        folder = dirs[ndir]
        if os.path.isdir(f'{path}{folder}') == False:
            continue
        # load metadata
        metadata = json.load(open(f'{path}{folder}/metadata.json', 'r'))
        # load shap values
        importance = pd.read_csv(f'{path}{folder}/importance.csv')['importance'][::-1]
        ...
        for i, k in enumerate(np.arange(0.05, 0.55, 0.05)):
            ...
            aopc[ndir, i] = abs(pred-perturbed_pred)
            log_odds[ndir, i] = np.log(perturbed_pred/pred) if round(pred, 0) == 1 else np.log((1-perturbed_pred)/(1-pred))

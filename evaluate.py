...
def calc_metrics(model_name='BERT', dataset='IMDB'):
    path = 'experiments/HEDGE/test/bert/'
    # load the model
    if model_name == 'BERT':
    ...
    for entry in os.scandir(path):
        if entry.is_dir():
            ...
            # load mobius transforms into a dict
            m = pd.read_csv(f'{path}{folder}/mobius_transforms.csv')
            m = dict(zip(m['term'], m['value']))
            # only select mobius transform for bottom layer (i.e singletons)
            importance = [abs(value) for key, value in m.items() if len(key.split('.')) == 1]
            importance.reverse()
            ...
            for i in range(Q):
                ...
                perturbed_pred = f(data)[0][0]
                coherence += abs(pred-perturbed_pred)
    ...
    aopc.mean().to_csv(f'experiments/HEDGE/bert/aopc.csv', index=False, header=False)
    log_odds = pd.DataFrame(log_odds, columns=columns)
    log_odds.mean().to_csv(f'experiments/HEDGE/bert/log_odds.csv', index=False, header=False)
    coherence = pd.DataFrame(coherence, columns=['coherence'])
    coherence.mean().to_csv(f'experiments/HEDGE/bert/coherence.csv', index=False, header=False)
